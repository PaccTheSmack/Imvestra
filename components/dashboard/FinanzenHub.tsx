...
const URGENCY_BADGE: Record<UrgencyKind, { bg: string; text: string; label: string }> = {
  ...
  ok:       { bg: "rgba(29,184,122,0.1)",  text: "#1DB87A", label: "Lauft"      },
};
...
export default function FinanzenHub({
  ...
  const summaryCards = [
    ...
    { label: "IST-EINNAHMEN",  value: formatCurrency(istEinnahmen),              color: istEinnahmen >= sollEinnahmen ? "#1DB87A" : "#FFB800" },
    { label: "AUSGABEN",       value: formatCurrency(ausgabenMonth),             color: "#FF4444" },
    { label: "NET CASHFLOW",   value: formatCurrencySigned(netCashflow),         color: netCashflow >= 0 ? "#1DB87A" : "#FF4444" },
  ];
  ...
  return (
    <div className="p-6 max-w-[1100px] mx-auto" style={{ minHeight: "100vh" }}>
      ...
      <div className="flex gap-0 mb-6" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        {TABS.map(({ id, label }) => (
          <button
            ...
            style={{ color: activeTab === id ? "#1DB87A" : "#444" }}
              ...
              >
                ...
                <div className="rounded-full h-2 overflow-hidden" style={{ background: "#1A1A1A" }}>
                  <motion.div
                    ...
                    style={{ background: sollCoveredPct >= 100 ? "#1DB87A" : "#FFB800" }}
                  />
                </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="text-right">
                              <p className="text-sm font-semibold" style={{ color: paid ? "#1DB87A" : "#FFB800" }}>
                                {formatCurrency(p.amount)}
                              </p>
                              <p className="text-[10px] mt-0.5" style={{ color: paid ? "#1DB87A" : "#FFB800" }}>
                                {p.status === "paid" ? "Bezahlt" : p.status === "late" ? "Ubfallig" : "Ausstehend"}
                              </p>
                            </div>
                          </div>
                        );
              ...
              >
                    ...
                    <div key={label} className="flex-1 flex flex-col items-center gap-1">
                      <div className="w-full flex items-end gap-0.5" style={{ height: 80 }}>
                        <motion.div
                          ...
                          style={{ background: "#1DB87A", opacity: 0.75, minHeight: income > 0 ? 2 : 0 }}
                        />
                        <motion.div
                          initial={prefersReduced ? {} : { height: 0 }}
                ...
                <div className="flex items-center gap-4 mt-4">
                  <div className="flex items-center gap-1.5">
                    <div className="w-2.5 h-2.5 rounded-sm" style={{ background: "#1DB87A", opacity: 0.75 }} />
              ...
              <div className="px-6 py-5 flex flex-col gap-4">
                ...
                {newMonthlyRate !== null && (
                  <motion.div
                    ...
                    style={{ background: "rgba(29,184,122,0.06)", border: "1px solid rgba(29,184,122,0.15)" }}
                  >
                    <p className="text-[10px] uppercase tracking-wide mb-1" style={{ color: "#555" }}>NEUE RATE / MONAT</p>
                    <p className="text-2xl font-semibold" style={{ color: "#1DB87A" }}>
                    ...
                    <div className="flex justify-between text-xs mt-3" style={{ color: "#555" }}>
                      ...
                      <span style={{ color: newMonthlyRate - selectedFinancing.rate_monthly > 0 ? "#FF4444" : "#1DB87A" }}>
